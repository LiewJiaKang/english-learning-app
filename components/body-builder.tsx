...
export function BodyBuilder({
  ...
  const activeBurger =
    burgers.find((burger) => burger.id === activeId) ??
    burgers[0]
  ...
  function changeBurgerType(
    ...
    setBurgers((current) =>
      current.map((burger) => {
        ...
        const first =
          burger.points[0] ?? emptyPoint()

        const second =
          burger.points[1] ?? emptyPoint()
  ...
  function updateEditingValue(value: string) {
    ...
    if (
      editing.field === "topicSentence" ||
      editing.field === "linkingSentence"
    ) {
      updateBurger(editing.burgerId, {
        [editing.field]: value,
      })
    } else if (
      editing.pointIndex !== undefined
    ) {
      updatePoint(
        editing.burgerId,
        editing.pointIndex,
  ...
  const activeIndex =
    burgers.findIndex(
      (burger) => burger.id === activeBurger.id
    ) + 1
                    ...
                    {burgers.length > 1 &&
                      !complete && (
                        <span
                          ...
                          className="hidden rounded p-1 text-muted-foreground hover:bg-muted hover:text-destructive group-hover:block"
                        >
        ...
        <section className="min-w-0">
          ...
          <div className="border-b px-4 py-4 sm:px-6">
            <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
              <div>
                <p className="text-xs text-muted-foreground">
                  Paragraph {activeIndex}
                </p>

                <h3 className="mt-1 font-semibold">
                  Build your cheeseburger
                </h3>
              </div>
          ...
          <div className="p-4 sm:p-6">
            <div className="mx-auto max-w-2xl space-y-2">
              ...
              {activeBurger.points.map(
                (point, index) => (
                  <div key={index}>
                    {/* Patty number for double burger */}
                    ...
                    <BurgerLayer
                      ...
                      onClick={() =>
                        openEditor(
                          activeBurger.id,
                          "point",
                          `🍗 Chicken Patty · Point${activeBurger.type ===
                            "double"
                            ? ` ${index + 1}`
                            : ""
                          }`,
                    ...
                    <BurgerLayer
                      ...
                      onClick={() =>
                        openEditor(
                          activeBurger.id,
                          "elaboration",
                          `🥬 Lettuce · Elaboration${activeBurger.type ===
                            "double"
                            ? ` ${index + 1}`
                            : ""
                          }`,
                    ...
                    <BurgerLayer
                      ...
                      onClick={() =>
                        openEditor(
                          activeBurger.id,
                          "example",
                          `🧀 Cheese · Example${activeBurger.type ===
                            "double"
                            ? ` ${index + 1}`
                            : ""
                          }`,
              ...
              <div className="mt-5 rounded-lg border border-dashed bg-muted/20 p-3">
                <p className="text-xs leading-5 text-muted-foreground">
                  ...
                  Topic sentence → Point → Elaboration
                  → Example → Linking sentence.
                  A double burger repeats the Point,
                  Elaboration, and Example layers.
                </p>
              </div>
            </div>
          </div>
